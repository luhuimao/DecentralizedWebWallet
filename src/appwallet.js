    ...
    return(
      <div style={{width:'100%',height:'80.5px'}}>
        <div style={{height:'80px',width:'100%',display: 'inline-flex'}}>
          ...
          <div style={{width:'100%',float: 'right',height:'80px',display: 'inline-flex'}}>
    ...
    if (weekArray && weekArray.length > 0) {
      ...
      try {
        ...
        for(i = 0; i < info.TokenInfos.length; i++) {
          //alert('tokenpubkey==' + info.TokenInfos[i].tokenpubkey);
          var tokenpubkey = new web3.PubKey(info.TokenInfos[i].tokenpubkey);
...
export class Wallet extends React.Component {
    ...
    for (var i = 0; i < array.length; i++) {
      ...
      if (tokens.tokenselected == true) {
        ...
        if (tokenaccpubkey.length == 0){
          var newtokenaccpubkey =  await token.createNewAccount(this.web3solAccount);
        ...
        for (var i = 0; i < this.state.tokenNameArray.length; i++) {
          ...
          if (tokens.tokenselected == true) {
            ...
            if (tokenaccpubkey.length == 0){
              var newtokenaccpubkey =  await token.createNewAccount(this.web3solAccount);
              tokens.tokenaccpubkey = newtokenaccpubkey.toString();
              const newTokenAccountInfo = await token.fetchAccountDetail(newtokenaccpubkey);
              tokens.tokenamount = newTokenAccountInfo.amount.toString();
            }else{
              var accpublickey = new web3.PubKey(tokenaccpubkey);
              const newTokenAccountInfo = await token.fetchAccountDetail(accpublickey);
              tokens.tokenamount = newTokenAccountInfo.amount.toString();
            }
          }
          tokensArr.push(tokens);
  ...
  requestAirdrop() {
    this.runModal(
      ...
      async () => {
        ...
        this.setState({
          balance: await this.web3sol.getBalance(this.web3solAccount.pubKey),
        });
      }
    );
  }
  ...
  sendTransaction() {
    this.runModal(
      ...
      async () => {
        ...
        this.setState({
          balance: await this.web3sol.fetchAccountBalance(this.web3solAccount.pubKey),
          tokenAmount: await this.web3sol.fetchAccountBalance(this.web3solAccount.pubKey)
        });
      }
    );
  }
  ...
  transferToken() {
    ...
    if (this.state.tokenAccPubkey == this.web3solAccount.pubKey.toString()) {
    ...
    this.runModal(
      ...
      '请稍后...',
      async () => {
        var token = await new web3.Token(this.web3sol, new web3.PubKey(this.state.tokenPublicKey));
        var sourcetokenacc = await token.fetchAccountDetail(new web3.PubKey(this.state.tokenAccPubkey));
        ...
        const sig = await token.transfer(
          this.web3solAccount,
          new web3.PubKey(this.state.tokenAccPubkey),
          new web3.PubKey(this.state.destTokenAccountPublicKey),
          this.state.transferTokenAmount
        );
        await this.web3sol.confmTxn(sig);
        sourcetokenacc = await token.fetchAccountDetail(new web3.PubKey(this.state.tokenAccPubkey));
        var desttokenacc = await token.fetchAccountDetail(new web3.PubKey(this.state.destTokenAccountPublicKey));
        this.setState({transferModal: false,showText:'发送成功',showModal: true});
        this.setState({
  ...
  pushNewWindow(index){
    if (index == 999) {
      this.setState({tokenName: 'BUS',tokenAmount:this.state.balance,tokenAccPubkey:this.web3solAccount.pubKey.toString(),tokenLogo: 'account_head.png',tokenPublicKey:this.web3solAccount.pubKey.toString()});
    }else{
